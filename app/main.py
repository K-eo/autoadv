...
c.execute("""CREATE TABLE IF NOT EXISTS keys(
    ...
    created_at TEXT, used INTEGER DEFAULT 0, user_id TEXT)""")
...
def redeem_key(key, user_id):
    ...
    if not result:
        return None
    plan, accs, duration_hours = result["plan"], result["accs"], result["duration_hours"]
...
class AccountSetupModal(Modal):
    def __init__(self, acc_number, setup_message, setup_view):
        ...
        self.channel_name = TextInput(label="Channel Names (comma-separated)", style=TextStyle.short)
            ...
            if res.get("status") == "ok":
                for item in self.setup_view.children:
                    if isinstance(item, Button) and item.label == f"Setup Acc {self.acc_number}":
                        item.disabled = True
                await self.setup_message.edit(view=self.setup_view)
                await interaction.response.send_message(f"Account {self.acc_number} locked and started.", ephemeral=False)
            else:
                c.execute("UPDATE accounts SET configured=0 WHERE user_id=? AND acc_number=?", (user_id, self.acc_number))
                conn.commit()
                await interaction.response.send_message(f"Error: {res.get('reason')}", ephemeral=False)
        except discord.errors.InteractionResponded:
            try:
                await interaction.followup.send(f"Error: {res.get('reason')}", ephemeral=False)
            except:
                pass
...
async def create_setup_channel(user: discord.User, accs: int, old_ticket=None):
    ...
    try:
        with open(os.path.join(BASE_DIR, "instructions.txt"), "r", encoding="utf-8") as f:
            instructions_text = f.read()
    except:
        instructions_text = "No instructions available."
    await channel.send(f"<@{user.id}>")
    await channel.send(embed=discord.Embed(title="Setup Instructions", description=instructions_text, color=discord.Color.green()))
    view = View(timeout=None)
    setup_message = await channel.send(embed=discord.Embed(
        title="Setup Here",
        description="Click a button to configure your accounts.",
...
async def panel(interaction: Interaction):
    if interaction.user.id not in ADMIN_IDS:
        ...
        return
    try:
        with open(os.path.join(BASE_DIR, "panel.txt"), "r", encoding="utf-8") as f:
            panel_text = f.read()
    except:
        panel_text = "No panel content."
...
async def gen_key(interaction: Interaction, duration: int, accs: int):
    if interaction.user.id not in ADMIN_IDS:
        await interaction.response.send_message("Not authorized.", ephemeral=True)
        return
    key = generate_key()
...
async def redeem(interaction: Interaction, key: str):
    ...
    for ch in interaction.guild.text_channels:
        if ch.name.startswith(f"ticket-{interaction.user.name.lower()}"):
            old_ticket = ch
            break
    await create_setup_channel(interaction.user, accs=result["accs"], old_ticket=old_ticket)
    try:
